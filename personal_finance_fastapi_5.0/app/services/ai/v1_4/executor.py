...
from app.services.ai.v1_4.tool_registry import get_tool
...
def execute_tool(tool_name: str, tool_input: dict[str, Any], user_id: int) -> Dict[str, Any]:
    ...
    logger_info(f"[AI_V1_4] executor执行工具 | tool:{tool_name} | input:{final_tool_input}")

    try:
        result = tool.invoke(final_tool_input)
        logger_info(f"[AI_V1_4] executor执行成功 | tool:{tool_name} | result:{result}")
        return result
    except Exception as e:
        logger_error(f"[AI_V1_4] executor执行失败 | tool:{tool_name} | input:{final_tool_input} | error:{repr(e)}")
